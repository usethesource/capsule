...
public abstract class AbstractVectorGenerator<T extends Vector.Immutable>
    extends ComponentizedGenerator<T> {

  private Class<T> target;
  private Size sizeRange;

  public AbstractVectorGenerator(Class<T> target) {
    super(target);
    this.target = target;
  }
...
//      } else
  ...
  /*
   * TODO: ArrayList<>(larger) does not work because Vector is not a collection type.
   */
//  @Override
...
//  }
  ...
  private T convert(List<?> items) {
    ...
    for (Object item : items) {
      converted = (T) converted.pushBack(item);
    }
    return converted;
  }
