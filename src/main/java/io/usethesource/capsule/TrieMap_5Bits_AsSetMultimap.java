...
public class TrieMap_5Bits_AsSetMultimap<K, V> implements ImmutableSetMultimap<K, V> {
  ...
  private static final TrieMap_5Bits_AsSetMultimap EMPTY_MAP =
      new TrieMap_5Bits_AsSetMultimap(CompactMapNode.EMPTY_NODE, 0, 0);
  ...
  TrieMap_5Bits_AsSetMultimap(AbstractMapNode<K, V> rootNode, int hashCode, int cachedSize) {
  ...
  public ImmutableSet<V> get(final Object o) {
    try {
      ...
      if (result.isPresent()) {
        return AbstractSpecialisedImmutableSet.setOf(result.get());
      } else {
        return null;
      }
    } catch (ClassCastException unused) {
  ...
  public ImmutableSetMultimap<K, V> __put(final K key, final V val) {
    ...
    if (details.isModified()) {
      if (details.hasReplacedValue()) {
        ...
        return new TrieMap_5Bits_AsSetMultimap<K, V>(newRootNode,
            hashCode + ((keyHash ^ valHashNew)) - ((keyHash ^ valHashOld)), cachedSize);
      }

      final int valHash = val.hashCode();
      return new TrieMap_5Bits_AsSetMultimap<K, V>(newRootNode, hashCode + ((keyHash ^ valHash)),
          cachedSize + 1);
    }
    ...
    if (keySet == null) {
      keySet = new AbstractSet<K>() {
        ...
        public Iterator<K> iterator() {
          return TrieMap_5Bits_AsSetMultimap.this.keyIterator();
        }
        ...
        @Override
        public boolean isEmpty() {
          return TrieMap_5Bits_AsSetMultimap.this.isEmpty();
        }
        ...
        public boolean contains(Object k) {
          return TrieMap_5Bits_AsSetMultimap.this.containsKey(k);
        }
      };
    }
    ...
    if (values == null) {
      values = new AbstractCollection<V>() {
        ...
        public boolean contains(Object v) {
          return TrieMap_5Bits_AsSetMultimap.this.containsValue(v);
        }
      };
    }
  ...
  public boolean equals(final Object other) {
    ...
    if (other instanceof TrieMap_5Bits_AsSetMultimap) {
      TrieMap_5Bits_AsSetMultimap<?, ?> that = (TrieMap_5Bits_AsSetMultimap<?, ?>) other;
  ...
  static final class TransientTrieMap_5Bits<K, V> implements TransientSetMultimap<K, V> {
    ...
    TransientTrieMap_5Bits(TrieMap_5Bits_AsSetMultimap<K, V> trieMap_5Bits) {
    ...
    public static class TransientSetMultimapKeyIterator<K, V> extends MapKeyIterator<K, V> {
      final TransientTrieMap_5Bits<K, V> collection;
      K lastKey;

      public TransientSetMultimapKeyIterator(final TransientTrieMap_5Bits<K, V> collection) {
    ...
    public static class TransientSetMultimapValueIterator<K, V> extends MapValueIterator<K, V> {
      final TransientTrieMap_5Bits<K, V> collection;

      public TransientSetMultimapValueIterator(final TransientTrieMap_5Bits<K, V> collection) {
        super(collection.rootNode);
        this.collection = collection;
      }
    ...
    public static class TransientSetMultimapEntryIterator<K, V> extends MapEntryIterator<K, V> {
      final TransientTrieMap_5Bits<K, V> collection;

      public TransientSetMultimapEntryIterator(final TransientTrieMap_5Bits<K, V> collection) {
        super(collection.rootNode);
        this.collection = collection;
      }
    ...
    public ImmutableSetMultimap<K, V> freeze() {
      ...
      mutator.set(null);
      return new TrieMap_5Bits_AsSetMultimap<K, V>(rootNode, hashCode, cachedSize);
    }
