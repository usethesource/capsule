...
public class TrieSetMultimap_HCHAMP<K, V> implements ImmutableSetMultimap<K, V> {
	...
	TrieSetMultimap_HCHAMP(AbstractSetMultimapNode<K, V> rootNode, int hashCode,
					int cachedSize) {
	...
	public static final <K, V> TransientSetMultimap<K, V> transientOf(Object... keyValuePairs) {
		...
		final TransientSetMultimap<K, V> result = TrieSetMultimap_HCHAMP.EMPTY_SETMULTIMAP
						.asTransient();
	...
	public ImmutableSetMultimap<K, V> __put(final K key, final V val) {
		...
		if (details.isModified()) {
			...
			return new TrieSetMultimap_HCHAMP<K, V>(newRootNode, hashCode
	...
	private Spliterator<ImmutableSet<V>> valueCollectionsSpliterator() {
		/*
		 * TODO: specialize between mutable / immutable ({@see
	...
	protected static abstract class AbstractSetMultimapNode<K, V> implements INode<K, V> {
		...
		abstract Optional<ImmutableSet<V>> findByKey(final K key, final int keyHash,
	...
	protected static abstract class CompactSetMultimapNode<K, V> extends
					AbstractSetMultimapNode<K, V> {
		...
		abstract CompactSetMultimapNode<K, V> copyAndSetValue(
						final AtomicReference<Thread> mutator, final int bitpos,
						final ImmutableSet<V> valColl);

		abstract CompactSetMultimapNode<K, V> copyAndInsertValue(
						final AtomicReference<Thread> mutator, final int bitpos, final K key,
						final ImmutableSet<V> valColl);

		abstract CompactSetMultimapNode<K, V> copyAndRemoveValue(
						final AtomicReference<Thread> mutator, final int bitpos);
		...
		static final <K, V> CompactSetMultimapNode<K, V> mergeTwoKeyValPairs(final K key0,
			...
			if (mask0 != mask1) {
				...
				if (mask0 < mask1) {
					return nodeOf(null, (int) (0), dataMap, new Object[] { key0, valColl0, key1,
									valColl1 });
				} else {
					return nodeOf(null, (int) (0), dataMap, new Object[] { key1, valColl1, key0,
									valColl0 });
				}
			} else {
				final CompactSetMultimapNode<K, V> node = mergeTwoKeyValPairs(key0, valColl0,
								keyHash0, key1, valColl1, keyHash1, shift + BIT_PARTITION_SIZE);
		...
		static {
			...
			EMPTY_NODE = new BitmapIndexedSetMultimapNode<>(null, (int) (0), (int) (0),
							new Object[] {});

		};

		static final <K, V> CompactSetMultimapNode<K, V> nodeOf(
		...
		CompactSetMultimapNode<K, V> updated(final AtomicReference<Thread> mutator, final K key,
			...
			if ((dataMap() & bitpos) != 0) { // inplace value
				...
				final CompactSetMultimapNode<K, V> subNodeNew = subNode.updated(mutator, key, val,
		...
		CompactSetMultimapNode<K, V> removed(final AtomicReference<Thread> mutator, final K key,
			...
			if ((dataMap() & bitpos) != 0) { // inplace value
					...
					if (valColl.contains(val)) {
						...
						if (valCollNew.size() == 0) { // earlier: arity() == 0
							if (this.payloadArity() == 2 && this.nodeArity() == 0) {
								/*
								 * Create new node with remaining pair. The new
								 * node will a) either become the new root
								 * returned, or b) unwrapped and inlined during
								 * returning.
								 */
								...
								if (dataIndex == 0) {
									return CompactSetMultimapNode.<K, V> nodeOf(mutator, (int) 0,
				...
				final CompactSetMultimapNode<K, V> subNode = nodeAt(bitpos);
				final CompactSetMultimapNode<K, V> subNodeNew = subNode.removed(mutator, key, val,
								keyHash, shift + BIT_PARTITION_SIZE, details);
	...
	protected static abstract class CompactMixedSetMultimapNode<K, V> extends
					CompactSetMultimapNode<K, V> {
	...
	private static final class BitmapIndexedSetMultimapNode<K, V> extends
					CompactMixedSetMultimapNode<K, V> {

		final AtomicReference<Thread> mutator;
		final Object[] nodes;

		private BitmapIndexedSetMultimapNode(final AtomicReference<Thread> mutator,
	...
	private static final class HashCollisionSetMultimapNode_BleedingEdge<K, V> extends
					CompactSetMultimapNode<K, V> {
			...
			for (int idx = 0; idx < keys.length; idx++) {
				if (keys[idx].equals(key)) {
					...
					if (currentValColl.contains(val)) {
						...
						final CompactSetMultimapNode<K, V> thisNew = new HashCollisionSetMultimapNode_BleedingEdge<>(
				...
				if (keys[idx].equals(key)) {
					...
					if (currentValColl.contains(val)) {
						...
						if (valCollNew.size() != 0) {
							...
							return new HashCollisionSetMultimapNode_BleedingEdge<>(keyHash, keys,
											valsNew);
						} else {
							// drop mapping
							if (this.arity() == 2) {
								/*
								 * Create root node with singleton element. This
								 * node will be a) either be the new root
								...
								return CompactSetMultimapNode.<K, V> nodeOf(mutator, nodeMap,
												dataMap, theOtherKey, theOtherVal);
							} else {
								@SuppressWarnings("unchecked")
								final K[] keysNew = (K[]) new Object[this.keys.length - 1];
								...
								System.arraycopy(this.keys, idx + 1, keysNew, idx, this.keys.length
												- idx - 1);
								...
								System.arraycopy(this.vals, idx + 1, valsNew, idx, this.vals.length
												- idx - 1);

								return new HashCollisionSetMultimapNode_BleedingEdge<>(keyHash,
												keysNew, valsNew);
							}
						}
					} else {
			...
			for (int idx = 0; idx < keys.length; idx++) {
				if (cmp.compare(keys[idx], key) == 0) {
					...
					if (currentValColl.contains(val)) {
						...
						// remove tuple
						final ImmutableSet<V> valCollNew = currentValColl.__removeEquivalent(val,
										cmp);
		...
		public boolean equals(Object other) {
			...
			HashCollisionSetMultimapNode_BleedingEdge<?, ?> that = (HashCollisionSetMultimapNode_BleedingEdge<?, ?>) other;
		...
		private boolean searchNextValueNode() {
			while (currentStackLevel >= 0) {
				...
				if (nodeCursor < nodeLength) {
					final AbstractSetMultimapNode<K, V> nextNode = nodes[currentStackLevel]
									.getNode(nodeCursor);
	...
	protected static class SetMultimapTupleIterator<K, V, T> extends
					AbstractSetMultimapIterator<K, V> implements Iterator<T> {
	 ...
	 * first recursively.
	 */
	private static class TrieSetMultimap_BleedingEdgeNodeIterator<K, V> implements
					Iterator<AbstractSetMultimapNode<K, V>> {
	...
	static final class TransientTrieSetMultimap_BleedingEdge<K, V> implements
					TransientSetMultimap<K, V> {
		...
		public static class TransientSetMultimapValueIterator<K, V> extends
						SetMultimapValueIterator<K, V> {
			final TransientTrieSetMultimap_BleedingEdge<K, V> collection;
		...
		public static class TransientSetMultimapTupleIterator<K, V, T> extends
						SetMultimapTupleIterator<K, V, T> {
			final TransientTrieSetMultimap_BleedingEdge<K, V> collection;
